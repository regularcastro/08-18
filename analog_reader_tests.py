...
def calibrate_gauge(gauge_number, file_type):
    ...
    img2 = cv2.imread("C:/Users/Castro/Desktop/Computa/TestesVC/08-18/mititoyosimpla.jpg")
    r = cv2.selectROI("select the area", img2) 
    
  
# Crop image 
    img = img2[int(r[1]):int(r[1]+r[3]),  
                      int(r[0]):int(r[0]+r[2])] 
    height, width = img.shape[:2]
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  #convert to gray
    gray = cv2.Canny(img, 1, 255)
...
# closing all open windows
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,255, np.array([]), 255, 20, int(height*0.35), int(width))
    print(circles)
    ...
    cv2.circle(img, (x, y), 2, (0, 0, 0), 3, cv2.LINE_AA)  # draw center of circle
...
def main():
    ...
    img = cv2.imread("C:/Users/Castro/Desktop/Computa/TestesVC/08-16/mititoyosimpla.jpg")
